...
lectures_mon = ["CSC", "GSS", "Mat 1o1"]
lectures_tue = ["Sci", "SSt", "IT"]
lectures_wed = ["Mat 1o1", "Sci", "English"]
lectures_thu = ["CSC", "english", "Mat 1o1"]
lectures_fri = ["Sci", "SSt", "IT"]
lectures_sat = ["Mat 1o1", "Sci", "English"]
...
if __name__ == "__main__":
    while True:
        ...
        if wochentage(t) == 'Saturday':
            if 6 <= hour < 24:
                engine.say(str(constant))
                engine.say("Today is a Saturday so you have only" + str(len(lectures_sat)) + "lectures")
                engine.say("They are as follows" + str(lectures_sat))
                engine.runAndWait()
